...
public class TokenProvider {
  ...
  public String generate(Authentication authentication, String email) {
    ...
    return Jwts.builder().header().type(TOKEN_TYPE).and()
        ...
        .audience().add(TOKEN_AUDIENCE).and().issuer(TOKEN_ISSUER).subject(user.getUsername())
        .claim("roles", roles).claim("email", email).compact();
  }
  ...
  public Optional<Jws<Claims>> validateTokenAndGetJws(String token) {
    try {
      ...
      log.error("Request to parse empty or null JWT : {} failed : {}", token,
          exception.getMessage());
    }
